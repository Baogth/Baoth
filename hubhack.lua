...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>thêm cho vui 🤧<Color=/>"):Display()
    wait(.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Hãy sử dụng Fluxus để có trải nghiệm tốt!!<Color=/>"):Display()
    wait(.24)
    require(game.ReplicatedStorage.Notification).new("<Color=Blue>Tham gia discord để cập nhật thông tin mới nhất nhé<Color=/>"):Display()
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>nhớ theo dõi tui nhé<Color=/>"):Display()
    wait(.36)
end)
...
W:AddLabel("PHÁT TRIỂN BỞI:"vũ đạt")
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "baowez(beta)(Noitification)",
                    Content = "[EN] : Please cancel the farrm level quest (if any)",
                    Image = "rbxassetid://8192162908",
                    Time = 9
                })
                OrionLib:MakeNotification({
                    Name = "baowez(Noitification)",
